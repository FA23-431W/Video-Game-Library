...
def print_formatted_game_list(gamelist):
    """
    Print game list in a formatted way.
    """
    if gamelist:
        header = "{:<8} | {:<12} | {:<40} | {:<15} | {:<8} | {:<10}".format(
            'Game ID', 'Publisher ID', 'Title', 'Category', 'Price', 'Release Date'
        )
        print("\n" + header)
        print("-" * len(header))

        for row in gamelist:
            print("{:<8} | {:<12} | {:<40} | {:<15} | {:<8} | {:<10}".format(*row))
    else:
        print("No game found.")
...
def view_game(conn, user_id):
    ...
    while True:
        ...
        if choice == "1":
            ...
            if gamelist:
                for row in gamelist:
                    print_formatted_game_list(gamelist)
            else:
                print("No game found.")
        elif choice == "2":
...
def view_specificDash(conn):
    try:
        ...
        cur.execute("SELECT * FROM Community c, Dashboard d WHERE c.communityID = %s AND c.dashboardID = d.dashboardID",
                    (communityID,))
        rows = cur.fetchall()
        return rows
    except mysql.connector.Error as e:
